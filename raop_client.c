...
static int encrypt1(raopcl_data_t *raopcld, uint8_t *data, int size)
{
	uint8_t *buf;
	//uint8_t tmp[16];
	int i=0,j;
...
static int raopcl_connectcontrol(raopcl_data_t *raopcld, uint32_t myport)
{
...
static int raopcl_connecttime(raopcl_data_t *raopcld, uint32_t myport)
{
...
/*uint64_t */ unsigned long long calculate_timestamp(struct timeval *tv) {
    uint64_t ntpts;
...
int raopcl_send_sample(raopcl_t *p, uint8_t *sample, int count)
{
	...
	if(raopcld->encrypt)
	{
		...
		encrypt1(raopcld, raopcld->data+header_size+audio_header_size, count);
	}
...
int raopcl_connect(raopcl_t *p, char *host,uint16_t destport, int encrypt, int volume)
{
	...
	sprintf(sid, "%d%hu", 3420, *((uint32_t*)buf));
